...
pub fn fake_payer_claim() -> PayerClaim {
    ...
    PayerClaim {
        claim_id: Faker.fake(),
        place_of_service_code: NumberWithFormat("##").fake::<String>().parse().unwrap_or(11),
        insurance: Insurance {
            payer_id: ["medicare", "united_health_group", "anthem"].choose(&mut rng).unwrap().to_string(),
        ...
        organization: Organization {
            ...
            ein: Some(format!("{}-{}", NumberWithFormat("##").fake::<String>(), NumberWithFormat("######").fake::<String>())),
        ...
        service_lines: vec![ServiceLine {
            ...
            modifiers: Some(vec![
                (0..2).map(|_| Word().fake::<String>()).collect::<Vec<_>>().join("")
            ]),
