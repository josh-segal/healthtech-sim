...
use healthtechsim::remittance::Remittance;
...
async fn test_remittance_amount_validation() {
    ...
    match history.get(&claim.claim_id) {
        Some(healthtechsim::message::ClaimStatus::Remitted(record)) => {
            ...
            assert!(remittance.validate_against_claim(&claim).is_ok(), 
                "Remittance should validate against claim");
            
            // Verify amounts add up to billed amount
            let billed_amount = claim.service_lines[0].unit_charge_amount * claim.service_lines[0].units as f64;
            ...
            assert!((total_remitted - billed_amount).abs() < 0.01, 
...
async fn test_claim_routing_to_correct_payer() {
    ...
    tokio::spawn(async move { medicare_payer.run().await; });
    tokio::spawn(async move { anthem_payer.run().await; });
    ...
    let history = remittance_history.lock().await;
    assert!(history.contains_key(&medicare_claim.claim_id), "Medicare claim should be processed");
    assert!(history.contains_key(&anthem_claim.claim_id), "Anthem claim should be processed");
...
async fn test_invalid_json_handling() {
    ...
    assert_eq!(received_claim_id, claim.claim_id, "Only the valid claim should be processed");
    
    // Verify only the valid claim is in history
    let history = remittance_history.lock().await;
    assert_eq!(history.len(), 1, "Only one claim should be in history");
    assert!(history.contains_key(&claim.claim_id), "Valid claim should be in history");
}
...
async fn test_concurrent_claim_processing() {
    ...
    assert_eq!(received_claim_ids.len(), 5, "All 5 claims should be processed");
    
    let history = remittance_history.lock().await;
    for claim in &claims {
        assert!(history.contains_key(&claim.claim_id), 
            "Claim {} should be in history", claim.claim_id);
    }
} 